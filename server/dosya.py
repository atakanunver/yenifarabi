"""server/dosya.py — dosya işleme (client/actions/file_processor.py'nin
ağır kısmı, Faz 2/8 server-taşıma planı).

`_process_*` fonksiyonlarının tamamı BİREBİR taşındı — yalnızca I/O sınırı
değişti: dosya artık `file_path` ile yerel diskte değil, multipart upload
ile geliyor; `_ai_metin`/`_ai_gorsel` artık HTTP proxy'ye değil (aynı
süreçte, aynı makinede olduğu için) doğrudan `saglayicilar` modülüne çağrı
yapıyor.

Üretilen dönüşüm dosyaları (`to_word`, `convert`, `resize`...) `server/gecici/
<request_id>/` altında tutulur, `/api/egitim/dosya_indir/...` ile indirilir,
24 saat sonra silinir — bunlar kalıcı bir kaynak değil, dersin anlık
ihtiyacı (mimari.md'nin "dosya içeriği DB'ye gömülmez" ilkesiyle aynı
ruhta: bu geçici üretilmiş dosyalar da DB'ye ya da kalıcı depoya girmez).
"""
...
def _eski_gecici_temizle() -> None:
    """Her istekte ucuz bir süpürme — 24 saatten eski request_id klasörlerini
    siler. Redis/cron gerektirmez (Kural 8), birkaç düzine klasörü stat'lamak
    ihmal edilebilir maliyet."""
    if not GECICI_DIR.exists():
        return
    esik = time.time() - ESKI_DOSYA_SAATI * 3600
    try:
        for d in GECICI_DIR.iterdir():
            if d.is_dir() and d.stat().st_mtime < esik:
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            if len(result) > 500 and params.get("save", True):
                ...
                return f"{result[:300]}...\n\nTam sonuç şuraya kaydedildi: {out.name}"
            return result
...
def _process_data(path: Path, file_type: str, action: str, params: dict, speak=None) -> str:
    ...
    try:
        df = pd.read_csv(path, encoding="utf-8", errors="replace") if file_type == "csv" else pd.read_excel(path)
    except Exception as e:
...
class DosyaIslemYanit(BaseModel):
    status: str  # "ok" | "hata" | "cok_buyuk" | "desteklenmiyor"
    sonuc: str | None = None
...
@router.post("/api/egitim/dosya_isle", response_model=DosyaIslemYanit)
async def dosya_isle(
    ...
    _eski_gecici_temizle()

    def _bitir(status: str, **kw) -> DosyaIslemYanit:
        return DosyaIslemYanit(status=status,
                                latency_ms=int((time.perf_counter() - t0) * 1000),
                                request_id=request_id, **kw)

    calisma_dizini = GECICI_DIR / request_id
    calisma_dizini.mkdir(parents=True, exist_ok=True)
    hedef = calisma_dizini / Path(dosya.filename or "dosya").name

    boyut = 0
    with open(hedef, "wb") as f:
        while True:
            parca = await dosya.read(1 << 20)
            if not parca:
                break
            boyut += len(parca)
            if boyut > YUKLEME_LIMIT_MB * 1024 * 1024:
                f.close()
                shutil.rmtree(calisma_dizini, ignore_errors=True)
                return _bitir("cok_buyuk", sonuc=f"Dosya {YUKLEME_LIMIT_MB}MB sınırını aşıyor.")
            f.write(parca)

    file_type = _detect_type(hedef)
    params = {"action": action, "instruction": instruction, "save": True}

    if file_type == "unknown":
        try:
            content = hedef.read_text(encoding="utf-8", errors="ignore")[:10000]
            prompt = (f"File: {hedef.name}\nContent preview:\n{content}\n\n"
                      f"Task: {action or instruction or 'Describe what this file contains and what can be done with it.'}")
            sonuc = _ai_metin(prompt)
            return _bitir("ok", sonuc=sonuc)
        except Exception as e:
            return _bitir("hata", sonuc=f"Bilinmeyen dosya türü ({hedef.suffix}). İşlenemedi: {e}")

    handler = _DISPATCH.get(file_type)
    if not handler:
        return _bitir("desteklenmiyor", sonuc=f"Desteklenmeyen dosya türü: {file_type}")

    try:
        sonuc = handler(hedef, action.lower().strip(), params, None) or "Done."
    except Exception as e:
        return _bitir("hata", sonuc=f"İşleme başarısız: {e}")

    # Handler bir çıktı dosyası üretmiş olabilir (aynı çalışma dizininde,
    # kaynak dosyadan farklı bir ad) — varsa indirme URL'ini yanıta ekle.
    uretilenler = [p for p in calisma_dizini.iterdir() if p != hedef]
    dosya_url = None
    if uretilenler:
        dosya_url = f"/api/egitim/dosya_indir/{request_id}/{uretilenler[0].name}"

    return _bitir("ok", sonuc=sonuc, dosya_url=dosya_url)


@router.get("/api/egitim/dosya_indir/{request_id}/{dosya_adi}")
def dosya_indir(request_id: str, dosya_adi: str):
    # Path traversal koruması: main.py'nin ders_kaydi_yedek endpoint'iyle
    # aynı disiplin — sabit GECICI_DIR'e göre containment kontrolü.
    if "/" in request_id or ".." in request_id or "/" in dosya_adi or ".." in dosya_adi:
        raise HTTPException(status_code=400, detail="Geçersiz yol")
    gecici_r = GECICI_DIR.resolve()
    yol = (GECICI_DIR / request_id / dosya_adi).resolve()
    if not yol.is_relative_to(gecici_r) or not yol.exists():
        raise HTTPException(status_code=404, detail="Dosya bulunamadı ya da süresi doldu")
    return FileResponse(yol, filename=dosya_adi)
